...
DAEMONCLASS=co.kr.istarbucks.xo.batch.main.PaymentVanillaCancel
DAEMONNAME=XO_PaymentVanillaCancel
DAEMONARGS1=$2
DAEMONARGS2=$3

CONSOLEFILE=$LOG/paymentVanillaCancel_console.log

JAVA_PNAME="-Dpn=$DAEMONNAME"
JAVA_PROPERTY="-Xms32m -Xmx128m"

if [ $# -eq 1 ]; then
      echo "start XO_PaymentVanillaCancel!"
      nohup $JAVA_COMMAND $JAVA_PNAME $JAVA_PROPERTY -cp $CLASSPATH $DAEMONCLASS > $CONSOLEFILE 2>&1 &
      exit 0
elif [ $# -eq 2 ]; then
      echo "start XO_PaymentVanillaCancel! [period] $DAEMONARGS1"
      nohup $JAVA_COMMAND $JAVA_PNAME $JAVA_PROPERTY -cp $CLASSPATH $DAEMONCLASS $1 $2 > $CONSOLEFILE 2>&1 &
      exit 0
elif [ $# -eq 3 ]; then
      echo "start XO_PaymentVanillaCancel! [period] $DAEMONARGS1 ~ $DAEMONARGS2"
      nohup $JAVA_COMMAND $JAVA_PNAME $JAVA_PROPERTY -cp $CLASSPATH $DAEMONCLASS $1 $2 $3 > $CONSOLEFILE 2>&1 &
      exit 0  
else
	echo "USAGE : paymentVanillaCancel.sh start"
fi

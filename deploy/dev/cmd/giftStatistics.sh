...
DAEMONCLASS=co.kr.istarbucks.xo.batch.main.GiftStatistics
DAEMONNAME=GiftStatistics

CONSOLEFILE=/sck/batch/logs/xo/gift_statistics_console.log

JAVA_PNAME="-Dpn=$DAEMONNAME"
JAVA_PROPERTY="-Xms32m -Xmx128m"

if [ $# -le 1 ]; then
      echo "start eGift-statistics"
      nohup $JAVA_COMMAND $JAVA_PNAME $JAVA_PROPERTY -cp $CLASSPATH $DAEMONCLASS $1 > $CONSOLEFILE 2>&1 &
      exit 0
else
	echo "USAGE : giftStatistics.sh yyyyMMdd"
fi

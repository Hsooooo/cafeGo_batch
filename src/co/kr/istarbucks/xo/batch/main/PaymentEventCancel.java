...
/**
 * 페어링 자동 결제 취소 PaymentEventCancel.
 * 페어링 주문이 오후 3시 이전까지 주문이 들어 오지않으면 취소 처리
 * @author namgu1
 * @since 2016. 6. 08.
...
//        this.empMgr = new EmpMgr(pcLogger, eLogger);
    ...
    public boolean processPaymentCancel ( Map<String, String> map, String logTitle ) {
        ...
        boolean empSuccess       = false;   // 임직원 할인 취소 요청 성공 여부
        boolean mGiftCancelSuccess = false; // 모바일 상품권 결제 취소 성공여부
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(orderDto.getEmp_no()) && StringUtils.isNotEmpty(orderDto.getEmp_auth_app_no())) {
                ...
                if (empResult.isEmpty()) {
                    ...
                    if ( this.pcLogger.isDebugEnabled () ) {
            			buf.delete (0, buf.length ()).append (logTitle).append ("step ").append ( ( step++ )).append (".임직원 할인 취소: ").append ( ( System.currentTimeMillis () - startTime )).append ("ms");
            			this.pcLogger.debug (buf.toString ());
            		}
                }
            }
         ...
         // 모바일 상품권 결제 취소 START ===========================================================
            ...
            // step 6. MSR : 회원의 등록된 스타벅스 카드 조회 START =============================================
            ...
            if(telCancelSuccess){
	            ...
	            // step 6. MSR : 회원의 등록된 스타벅스 카드 조회 END ===========================================
	            
	            
	            // step 7. FD : 사용 취소(reload) 처리 START ==================================================
	            startTime = System.currentTimeMillis ();
	            try {
	                boolean isOk = false;
            ...
            // step 8. MSR : e-Gift 카드 발행 END   ==========================================================
            

            // step 9. MSR : SBC 카드 잔액 변경 START =========================================================
            ...
            // step 9. MSR : SBC 카드 잔액 변경 END   =========================================================

            // step 10. XO : 주문정보 결제 취소 상태로 변경 START ================================================
            startTime = System.currentTimeMillis ();
            boolean isOk = false;
            ...
            // step 10. XO : 주문정보 결제 취소 상태로 변경 END   ==============================================
    ...
    private RefundSbcDto egiftCardPublish ( RefundSbcDto refundDto, String logTitle ) throws Exception {
        ...
        try {
            ...
            if ( StringUtils.isEmpty (cardNumber) || StringUtils.equals (cardNumber, "-1") ) { throw new XOException ("e-Gift 발급 가능한 카드가 부족"); }
    ...
    /**
     * 결제 취소 처리(신용카드, SSG PAY, SSG MONEY)
     * @param orderDto
     * @param paymentList
     * @param logTitle
     * @return
    ...
    public boolean processPaymentPgCancel ( Map<String, String> map, String logTitle ) {
        ...
        MGiftHttpService mGiftHttpService = new MGiftHttpService();	// 모바일 상품권
        try {
            // XO 트랜잭션 시작
            xoSqlMap.startTransaction ();
            ...
            // step 8. PG(SSG PAY 포함) : 결제 취소 START ===================================================================
        	if(telCancelSuccess){
        	...
        	// step 8. PG(SSG PAY 포함) : 결제 취소 END =====================================================================

        	
        	// step 9. MSR : 쿠폰 사용 취소 START =============================================================
            ...
            // step 10. 주문 정보 업데이트 START   ==============================================================
            if(telCancelSuccess && cancelSuccess) {	// 취소 성공인 데이터만 업데이트
            	startTime = System.currentTimeMillis ();
            	try {
            ...
            // step 10. 주문 정보 업데이트 END   ================================================================

            // step 11. 주문 정보 변경 이력 저장 START   =========================================================
            startTime = System.currentTimeMillis ();
            dbSuccess = false;
            try {
            ...
            // step 11. 주문 정보 변경 이력 저장 END   ===========================================================
            
            
            // step 12. 결제 정보 업데이트 START   ==============================================================
        	startTime = System.currentTimeMillis ();
        	for(PaymentDto paymentDto : paymentList){
        		//신용카드와 통신사제휴 결과 업데이트
